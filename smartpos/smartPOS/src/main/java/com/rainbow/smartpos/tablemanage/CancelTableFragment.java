...
public class CancelTableFragment extends Fragment implements OnClickListener {
    private View view;
    ...
    private void initView() {
        ...
        mAdapter = new TableManageAdapter(getActivity(), MainScreenActivity.CANCEL_TABLE_FRAGMENT);
        ...
        title.setText(getString(R.string.return_table));
        ...
        switch (v.getId()) {
            ...
            case R.id.sure_btn:
                if (mAdapter.getSelectPosition() != -1) {
                    ...
                    if (seatEntity.lock) {
                        Toast.makeText(activity,"桌子被锁",Toast.LENGTH_LONG).show();

                        return;
                    }
                    new ReminderDialog2(getActivity(), "确定要对“" + seatEntity.tableName + "”进行消台吗！", true, new ReminderDialog2.ReminderDialog2Listener() {
                        @Override
                        public void cancle() {
                        ...
                        @Override
                        public void confirm() {
                            SanyiScalaRequests.ChangeTableRequest(ChangeTableAction.CANCELTABLE, seatEntity.seat, SanyiSDK.getStaffId(), new ICallBack() {
...
//                    new ReminderDialog(getActivity(), "确定要对“" + seatEntity.tableName + "”进行消台吗！", new OnSureListener() {
//
//                        @Override
